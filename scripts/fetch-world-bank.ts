/**
 * Fetches CPI data (FP.CPI.TOTL) from the World Bank API for all countries.
 * Normalizes to base year 2015=100.
 * Outputs to public/data/world_cpi.json
 */
...
const OUT_FILE = resolve(OUT_DIR, "world_cpi.json");
...
export async function fetchWorldBankCPI(): Promise<void> {
  console.log("[World Bank] Fetching CPI data...");
  ...
  try {
    ...
    const output: CategoryDataFile = {
      ...
      source: "World Bank - FP.CPI.TOTL (Consumer Price Index, 2010=100)",
      countries,
    };
    ...
    const fallback: CategoryDataFile = {
      baseYear: BASE_YEAR,
      lastUpdated: new Date().toISOString().split("T")[0],
      source: "World Bank - FP.CPI.TOTL (UNAVAILABLE)",
      countries: {},
    };
